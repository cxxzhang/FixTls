...
decltype(LdrLoadDll) *pfn_LdrLoadDll = LdrLoadDll;
...
LdrpAllocateTls(PTEB _Teb)
{
    ...
    if (LdrpNumberOfTlsEntries) {
        ...
        TlsVector = (PVOID *)RtlAllocateHeap(ProcessHeap,
                                             MAKE_TAG(TLS_TAG),
                                             sizeof(PVOID) * (LdrpNumberOfTlsEntries + MAX_TLS_DYN_LOAD_SUPPORT)); //预留一些空间
        ...
        //
        RtlZeroMemory(TlsVector, sizeof(PVOID) * (LdrpNumberOfTlsEntries + MAX_TLS_DYN_LOAD_SUPPORT));
        ...
        while (Next != Head) {
            ...
            Next = Next->Flink;
            TlsVector[TlsEntry->Tls.Characteristics] = RtlAllocateHeap(ProcessHeap,
                                                                       MAKE_TAG(TLS_TAG),
                                                                       TlsEntry->Tls.EndAddressOfRawData
            ...
            RtlCopyMemory(TlsVector[TlsEntry->Tls.Characteristics],
    ...
    if (TlsVector) {
        ...
        while (Next != Head) {
            ...
            if (TlsVector[TlsEntry->Tls.Characteristics]) {
                ...
                RtlFreeHeap(ProcessHeap, 0, TlsVector[TlsEntry->Tls.Characteristics]);
            }
        }
...
PVOID NTAPI TlsQuerySystemInformation(_In_ SYSTEM_INFORMATION_CLASS SystemInformationClass, _Out_opt_ LPDWORD ReturnLength)
{
    ...
    do {
        ...
        if (len) {
            len *= 2;
            buffer = RtlAllocateHeap(RtlProcessHeap(), 0, len);
            if (!buffer)
                return nullptr;
        }

        status = NtQuerySystemInformation(SystemInformationClass, buffer, len, &len);
...
VOID TlsFixInAllThread()
{
    ...
    while (true) {
        ...
        if (!p_process_info->NextEntryOffset)
            break;
...
static VOID NTAPI tlsload_tls_callback(PVOID DllHandle, DWORD Reason, PVOID Reserved)
{
    switch (Reason) {
...
NTSTATUS
LdrpDynAddTls(PTEB _Teb, PIMAGE_TLS_DIRECTORY TlsImage)
{
    PTEB Teb = _Teb;
    PLDRP_TLS_ENTRY TlsEntry = nullptr;
    PVOID *TlsVector = (PVOID *)Teb->ThreadLocalStoragePointer;
    HANDLE  ProcessHeap = Teb->ProcessEnvironmentBlock->ProcessHeap;

    //
    // Allocate the array of thread local storage pointers
    //

    if (LdrpNumberOfTlsEntriesDynLoad >= MAX_TLS_DYN_LOAD_SUPPORT) {
        return STATUS_NO_MEMORY;
    }
    ...
    *(PLONG)TlsEntry->Tls.AddressOfIndex = LdrpNumberOfTlsEntries + LdrpNumberOfTlsEntriesDynLoad;   
    TlsEntry->Tls.Characteristics = LdrpNumberOfTlsEntries + LdrpNumberOfTlsEntriesDynLoad;
	LdrpNumberOfTlsEntriesDynLoad++;
...
VOID TlsAddDynModuleInAllThread(PIMAGE_TLS_DIRECTORY TlsImage)
{
    auto pid = NtCurrentProcessId();
    auto p_system_process_info = PSYSTEM_PROCESS_INFORMATION(TlsQuerySystemInformation(SystemProcessInformation, nullptr));
    if (!p_system_process_info) {
        ...
        if (p_process_info->UniqueProcessId == pid) {
            ...
            for (ULONG i = 0; i < p_process_info->NumberOfThreads; ++i) {
                ...
                if (NT_SUCCESS(status)) {
                    status = NtQueryInformationThread(hThread, ThreadBasicInformation, &tbi, sizeof(tbi), nullptr);
                    if (NT_SUCCESS(status) && !!tbi.TebBaseAddress->ThreadLocalStoragePointer) {
                        LdrpDynAddTls(tbi.TebBaseAddress, TlsImage);
                    }
...
NTSTATUS
NTAPI
hook_LdrLoadDll(IN OPTIONAL PWSTR DllPath, IN OPTIONAL PULONG DllCharacteristics, IN PUNICODE_STRING DllName, OUT PVOID *DllHandle)
{
    NTSTATUS status = pfn_LdrLoadDll(DllPath, DllCharacteristics, DllName, DllHandle);
    if (!NT_SUCCESS(status)) {
        return status;
    }

    HMODULE hModule = (HMODULE)*DllHandle;
    if (hModule == nullptr) {
        return status;
    }
    ...
    TlsImage = (PIMAGE_TLS_DIRECTORY)RtlImageDirectoryEntryToData(hModule, TRUE, IMAGE_DIRECTORY_ENTRY_TLS, &TlsSize);

    if (TlsImage && TlsImage->AddressOfIndex && *((PLONG)TlsImage->AddressOfIndex) == 0) {
        TlsAddDynModuleInAllThread(TlsImage);
    }
...
void FixTls()
{
    ...
    DetourAttach(&(PVOID &)pfn_LdrLoadDll, hook_LdrLoadDll);
